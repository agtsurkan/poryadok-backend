...
Фронт — standalone-бандл Claude Design (не обычный репозиторий), это **отдельная задача**
(бриф §11.4). Бэкенд намеренно говорит на его же ключах, поэтому достаточно заменить
чтение/запись `localStorage` на `GET/PUT /state` (плюс экран логина). CORS уже включён.
